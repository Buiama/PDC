...
//    private static final ExecutorService executorService = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
    ...
    public static void main(String[] args) {
        String path = "C:\\Users\\Lenovo\\Documents\\KPI\\III курс\\ТПО\\Data\\";
    ...
    private static Runnable createSimpleVectorTask(double[] B, double[][] MC, double[] D, double[][] MM, int amount) {
        return () -> {
    ...
    private static Runnable createKahanVectorTask(double[] B, double[][] MC, double[] D, double[][] MM, int amount) {
        return () -> {
            long startTime = System.currentTimeMillis();
    ...
    private static Runnable createKahanMatrixTask(double[][] MC, double[][] MZ, double[] B, double[] D, double[][] MM, double a, int amount) {
        return () -> {
            final CyclicBarrier barrier = new CyclicBarrier(3);
            double[][][] matrix = new double[2][][];
            double[][] MF_Kahan;
    ...
    private static Runnable createKahanBabushkaVectorTask(double[] B, double[][] MC, double[] D, double[][] MM, int amount) {
        return () -> {
            long startTime = System.currentTimeMillis();
    ...
    private static Runnable createKahanBabushkaMatrixTask(double[][] MC, double[][] MZ, double[] B, double[] D, double[][] MM, double a, int amount) {
        return () -> {
            final CyclicBarrier barrier = new CyclicBarrier(3);
            double[][][] matrix = new double[2][][];
    ...
    private static class RowResult {
        ...
        public RowResult(int row, int column, double value) {
            this.row = row;
            this.column = column;
            this.value = value;
        }
    ...
    public static double[][] multiply(double[][] a, double[][] b) {
        ...
        BlockingQueue<RowResult> queue = new ArrayBlockingQueue<>(1000);
        for (int i = 0; i < a.length; i++) {
            final int row = i;
            executorService.submit(() -> {
                for (int j = 0; j < b[0].length; j++) {
                    ...
                    try {
                        queue.put(new RowResult(row, j, sum));
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt();
                    }
                }
        ...
        for (int count = 0; count < a.length * b[0].length; count++) {
            RowResult resultEntry;
            try {
                resultEntry = queue.take();
            } catch (InterruptedException e) {
                throw new RuntimeException(e);
            }
            result[resultEntry.row][resultEntry.column] = resultEntry.value;
        }
        ...
        for (int i = 0; i < a.length; i++) {
            ...
            executorService.submit(() -> {
                for (int j = 0; j < b[0].length; j++) {
                    ...
                    try {
                        queue.put(new RowResult(row, j, sum + cs + ccs));
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt();
                    }
                }
