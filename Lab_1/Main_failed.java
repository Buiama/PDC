...
//        double timeMatrixSimple, timeMatrixKahan, timeMatrixKahanBabushka;
        ...
        Thread threadVectorSimple = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                double[] C = subtract(multiplyParallel(MC, B), multiplyParallel(MM, D));
...
//                }
        ...
        Thread threadMatrixSimple = new Thread(new Runnable() {
            ...
            public void run() {
                long startTime = System.currentTimeMillis();
                double[][] MF = add(multiplyParallel(multiplyParallel(MC, MZ), min(add(B, D))), multiplyParallel(multiplyParallel(MM, add(MC, MM)), a));
...
//                }
        ...
        Thread threadVectorKahan = new Thread(new Runnable() {
            ...
            public void run() {
                long startTime = System.currentTimeMillis();
                double[] C_Kahan = subtractKahan(multiplyKahanParallel(MC, B), multiplyKahanParallel(MM, D));
...
//                }
        ...
        Thread threadMatrixKahan = new Thread(new Runnable() {
            ...
            public void run() {
                long startTime = System.currentTimeMillis();
                double[][] MF_Kahan = addKahan(multiply(multiplyKahanParallel(MC, MZ), min(addKahan(B, D))), multiply(multiplyKahanParallel(MM, addKahan(MC, MM)), a));
...
//                }
        ...
        Thread threadVectorKahanBabushka = new Thread(new Runnable() {
            ...
            public void run() {
                long startTime = System.currentTimeMillis();
                double[] C_KahanBabushka = subtractKahanBabushka(multiplyKahanBabushkaParallel(MC, B), multiplyKahanBabushkaParallel(MM, D));
...
//                }
        ...
        Thread threadMatrixKahanBabushka = new Thread(new Runnable() {
            ...
            public void run() {
                long startTime = System.currentTimeMillis();
                double[][] MF_KahanBabushka = addKahanBabushka(multiply(multiplyKahanBabushkaParallel(MC, MZ), min(addKahanBabushka(B, D))), multiply(multiplyKahanBabushkaParallel(MM, addKahanBabushka(MC, MM)), a));
...
//        writeToFile("MF_BigDecimal.txt", MF_BigDecimal);
    ...
    public static double[] multiplyParallel(double[][] matrix, double[] vector) {
    ...
    public static double[] multiplyKahanParallel(double[][] matrix, double[] vector) {
        final double[] result = new double[matrix.length];
        Thread[] threads = new Thread[matrix.length];
        ...
        for (int i = 0; i < matrix.length; i++) {
            ...
            threads[i] = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    double c = 0.0;
                    for (int j = 0; j < vector.length; j++) {
                        double y = matrix[row][j] * vector[j] - c;
                        double t = sum + y;
                        c = (t - sum) - y;
    ...
    public static double[][] multiplyKahanParallel(double[][] a, double[][] b) {
        final double[][] result = new double[a.length][b[0].length];
        Thread[] threads = new Thread[a.length];
    ...
    public static double[] multiplyKahanBabushkaParallel(double[][] matrix, double[] vector) {
        final double[] result = new double[matrix.length];
        Thread[] threads = new Thread[matrix.length];
        ...
        for (int i = 0; i < matrix.length; i++) {
            ...
            threads[row] = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (int j = 0; j < vector.length; j++) {
                        t = sum + matrix[row][j] * vector[j];
                        if (Math.abs(sum) >= Math.abs(matrix[row][j] * vector[j])) {
                            c = (sum - t) + matrix[row][j] * vector[j];
                        } else {
                            c = (matrix[row][j] * vector[j] - t) + sum;
                        }
                        sum = t;
                        t = cs + c;
                        if (Math.abs(cs) >= Math.abs(c)) {
    ...
    public static double[][] multiplyKahanBabushkaParallel(double[][] a, double[][] b) {
        final double[][] result = new double[a.length][b[0].length];
        Thread[] threads = new Thread[a.length];
        ...
        for (int i = 0; i < a.length; i++) {
            ...
            threads[row] = new Thread(() -> {
                for (int j = 0; j < b[0].length; j++) {
                    ...
                    double ccs = 0.0;
                    double cs = 0.0;
                    double cc, c, t = 0, product;
                    for (int k = 0; k < a[row].length; k++) {
                        product = a[row][k] * b[k][j];
                        t = sum + product;
                        if (Math.abs(sum) >= Math.abs(product)) {
                            c = (sum - t) + product;
                        } else {
