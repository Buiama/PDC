...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        RecursiveAction vectorTaskSimple = new RecursiveAction() {
        ...
        RecursiveAction matrixTaskSimple = new RecursiveAction() {
            protected void compute() {
                long startTime = System.currentTimeMillis();
                RecursiveTask<double[][]> task1 = new RecursiveTask<>() {
                    protected double[][] compute() {
                        return multiply(multiply(MC, MZ), min(add(B, D)));
                    }
                };
                RecursiveTask<double[][]> task2 = new RecursiveTask<>() {
                    protected double[][] compute() {
                        return multiply(multiply(MM, add(MC, MM)), a);
                    }
                };

                task1.fork();
                task2.fork();
                double[][] MF = add(task1.join(), task2.join());
        ...
        RecursiveAction vectorTaskKahan = new RecursiveAction() {
            protected void compute() {
                long startTime = System.currentTimeMillis();
        ...
        RecursiveAction matrixTaskKahan = new RecursiveAction() {
            protected void compute() {
                long startTime = System.currentTimeMillis();
                RecursiveTask<double[][]> task1 = new RecursiveTask<>() {
                    protected double[][] compute() {
                        return multiply(multiplyKahan(MC, MZ), min(add(B, D)));
                    }
                };
                RecursiveTask<double[][]> task2 = new RecursiveTask<>() {
                    protected double[][] compute() {
                        return multiply(multiplyKahan(MM, add(MC, MM)), a);
                    }
                };

                task1.fork();
                task2.fork();
                double[][] MF_Kahan = add(task1.join(), task2.join());
        ...
        RecursiveAction vectorTaskKahanBabushka = new RecursiveAction() {
            protected void compute() {
                long startTime = System.currentTimeMillis();
                ...
                lock.lock();
                try {
                    System.out.println("\n\nVector C_Kahan-Babushka with modified threads:");
        ...
        RecursiveAction matrixTaskKahanBabushka = new RecursiveAction() {
            protected void compute() {
                long startTime = System.currentTimeMillis();
                RecursiveTask<double[][]> task1 = new RecursiveTask<>() {
                    protected double[][] compute() {
                        return multiply(multiplyKahanBabushka(MC, MZ), min(add(B, D)));
                    }
                };
                RecursiveTask<double[][]> task2 = new RecursiveTask<>() {
                    protected double[][] compute() {
                        return multiply(multiplyKahanBabushka(MM, add(MC, MM)), a);
                    }
                };

                task1.fork();
                task2.fork();
                double[][] MF_KahanBabushka = add(task1.join(), task2.join());
                timeMatrixKahanBabushka.set(System.currentTimeMillis() - startTime);
                lock.lock();
                try {
                    System.out.println("\nMatrix MF_Kahan-Babushka with modified threads:");
        ...
        pool.execute(vectorTaskSimple);
        pool.execute(matrixTaskSimple);
        pool.execute(vectorTaskKahan);
        pool.execute(matrixTaskKahan);
        pool.execute(vectorTaskKahanBabushka);
        pool.execute(matrixTaskKahanBabushka);

        try {
            vectorTaskSimple.get();
            matrixTaskSimple.get();
            vectorTaskKahan.get();
            matrixTaskKahan.get();
            vectorTaskKahanBabushka.get();
            matrixTaskKahanBabushka.get();
            pool.shutdown();
